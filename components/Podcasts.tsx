    ...
    >
      <div className="max-w-7xl mx-auto">
        <div className={`mb-20 transition-all duration-1000 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}>
          <p className="section-label-light mb-4">Listen</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-1">
          {podcasts.map((pod, i) => (
            <div
              key={pod.title}
              className={`group relative p-10 md:p-14 border border-white/10 hover:border-gold/30 transition-all duration-700 ${
                visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
              }`}
              style={{ transitionDelay: `${i * 150 + 100}ms` }}
            >
              <div className="mb-8">
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="text-gold opacity-60 group-hover:opacity-100 transition-opacity duration-500">
              ...
              <p className="section-label-light mb-3">{pod.handle}</p>
              <h3 className="font-cormorant text-cream font-light text-3xl md:text-4xl mb-5 group-hover:text-gold transition-colors duration-500">
                {pod.title}
              </h3>
              <p className="font-jost text-cream/70 text-base md:text-lg leading-relaxed mb-8">
                {pod.description}
              </p>
              ...
              {pod.comingSoon ? (
                ...
                >
                  <span className="font-jost text-sm tracking-widest uppercase text-cream/60 group-hover/link:text-gold transition-colors duration-300">
                    {pod.linkLabel}
                  </span>
                  <span className="w-8 h-px bg-cream/40 group-hover/link:bg-gold group-hover/link:w-12 transition-all duration-300" />
                </a>
              ) : null}
